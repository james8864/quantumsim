...
Note: This uses a Cartesian grid with Dirichlet BC (ψ=0 at the box boundary). Orbital shapes
(sphere, dumbbell, etc.) are approximate: true s, p, d shapes come from spherical harmonics.
Box size is scaled with 1/Z so heavier atoms (smaller orbitals) are resolved correctly.
"""
import numpy as np
from scipy import sparse
from scipy.sparse.linalg import eigsh

from potential_3d import potential_on_grid, _nuclei_to_charges_positions


def _bounding_box(nuclei, padding=3.0):
    """
    Return (x_min, x_max, y_min, y_max, z_min, z_max).
    For a single nucleus: scale box with 1/Z (Bohr radius ~ 1/Z) so orbitals are
    well resolved and not clipped—larger Z → smaller box. For multiple nuclei use
    padding around their extent.
    """
    if not nuclei:
        return -4, 4, -4, 4, -4, 4
    Z_arr, pos = _nuclei_to_charges_positions(nuclei)
    xs, ys, zs = pos[:, 0], pos[:, 1], pos[:, 2]
    cx = (float(np.min(xs)) + float(np.max(xs))) / 2
    cy = (float(np.min(ys)) + float(np.max(ys))) / 2
    cz = (float(np.min(zs)) + float(np.max(zs))) / 2

    if len(nuclei) == 1:
        # Single atom: characteristic length ~ 1/Z. Use half-width ~ 18/Z so 1s, 2s, 2p
        # extend well inside the box (less box-boundary effect).
        Z = float(Z_arr[0])
        half = max(18.0 / max(Z, 0.5), padding)
    else:
        extent = max(
        ...
        # For molecules, ensure box is at least ~18/min(Z) so orbitals have room
        min_Z = float(np.min(Z_arr))
        half = max(extent / 2 + padding, 12.0 / max(min_Z, 0.5))
    return (
        cx - half, cx + half,
        cy - half, cy + half,
        cz - half, cz + half,
...
def solve_schrodinger(nuclei, n_grid=26, num_states=8):
    """
