...
class PubChemChatbot:
    ...
    def _on_show_schrodinger_cloud(self):
        ...
        self._bot_info(
            f"Running Schrödinger solver (50×50×50, Hartree SCF) for {mol_name}…\n"
            "Typically ~2–5 min depending on molecule size.")

        def run():
            try:
                data = compute_schrodinger_cloud(
                    nuclei, num_electrons=self._current_mol["total_electrons"])
    ...
    def _on_show_energy_levels(self):
        ...
        self._bot_info(
            f"Computing {num_orbitals} eigenvalues (50×50×50, Hartree SCF) for {mol_name}…\n"
        ...
        def run():
            try:
                data = compute_schrodinger_cloud(
                    nuclei, num_states=num_orbitals,
