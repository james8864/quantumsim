"""
Schrödinger solver — 50×50×50 grid (125k points), l_span=20 Bohr, dx=0.4.

Potential + kinetic + Hartree (e-e Coulombic repulsion). SCF loop iterates
until density converges. Electron shielding increases effective radius of cloud.
...
Compute note: 125k×125k eigsh + Hartree SCF; benzene ~2–4 min
depending on molecule size and convergence.
"""
...
# ─────────────────────  Default parameters  ─────────────────────
# 50×50×50 = 125,000 grid points; span 20 Bohr → dx = 0.4 Bohr
DEFAULT_N = 50
DEFAULT_L = 20.0
NUM_EIGENVALUES = 10
SCF_MAX_ITERS = 10
...
def _hartree_potential(density_3d, dx):
    """
    Coulombic e-e repulsion: V_H(r) = ∫ ρ(r')/|r-r'| d³r'.
    Solves ∇²V = -4πρ in Fourier space (FFT Poisson).
    """
...
def compute_schrodinger_cloud(nuclei, n=None, l_span=None, num_states=None,
                              num_electrons=None):
    """
    Run full Schrödinger pipeline: grid → V → [SCF: H → eigsh → ψ → ρ → V_H] → psi.

    nuclei : [(symbol, (x,y,z)), ...]  in Bohr
    n      : grid points per axis (default 80)
    l_span : total span per axis in Bohr (default 20) → dx = l_span/n
    num_states : number of eigenvalues (default 10)
    num_electrons : total electrons for Hartree SCF (None = skip e-e repulsion)

    Returns dict with: x_flat, y_flat, z_flat, nuclei, V_vec, prob_list,
    prob_total, eigenvalues, n, n_total, x_1d, y_1d, z_1d
    """
    if not nuclei:
        return None
    n = n or DEFAULT_N
    ...
    num_e = num_electrons if num_electrons is not None else 0
    run_hartree = num_e >= 2
    ...
    V_nuc = V_func(x_flat, y_flat, z_flat)
    V_mat = sparse.diags(V_nuc, 0, shape=(n_total, n_total), format="csr")
    ...
    def _fill_occupancies(ne, norb):
        """Pauli: 2e per orbital. Returns list of occupancies per orbital."""
        occ = []
        rem = ne
        for _ in range(norb):
            o = min(2, rem)
    ...
    V_H_flat = np.zeros(n_total)
    density_old = None
    n_iters = SCF_MAX_ITERS if run_hartree else 1

    for it in range(n_iters):
        H_mat = T_mat + V_mat + sparse.diags(V_H_flat, 0, format="csr")
        eigenvalues, eigenvectors = eigsh(H_mat, k=num_states, which="SA")

        psi_flat = [eigenvectors[:, i].copy() for i in range(num_states)]
        occupancies = _fill_occupancies(num_e, num_states)

        # Normalize
        for i in range(num_states):
            norm = np.sqrt(np.sum(psi_flat[i]**2) * dV)
            if norm > 1e-30:
                psi_flat[i] /= norm

        prob_list = [np.abs(psi_flat[i]) ** 2 for i in range(num_states)]
        density_new = np.zeros(n_total)
        for i, occ in enumerate(occupancies):
            if occ > 0:
                density_new += occ * prob_list[i]
        density_new = density_new.reshape((n, n, n), order="C")

        if run_hartree and it < n_iters - 1:
            if density_old is not None:
                density_mix = SCF_MIX * density_new + (1.0 - SCF_MIX) * density_old
            else:
                density_mix = density_new.copy()

            # Converge check before update
            if density_old is not None:
                diff = np.abs(density_mix - density_old)
                if np.max(diff) < SCF_TOL:
                    break

            density_old = density_mix.copy()

            V_H_3d = _hartree_potential(density_mix, dx)
            # Fermi–Amaldi: self-interaction correction
            V_H_3d *= (num_e - 1.0) / num_e
            V_H_flat = V_H_3d.ravel(order="C")

    prob_total = np.sum(prob_list, axis=0)
    V_vec = V_nuc + V_H_flat
...
if __name__ == "__main__":
    ...
    # Estimate electrons from sum of Z (neutral molecule)
    from potential_3d import SYMBOL_TO_Z
    num_e = sum(SYMBOL_TO_Z.get(s, 1) for s, _ in nuclei)
    data = compute_schrodinger_cloud(nuclei, num_electrons=num_e)
